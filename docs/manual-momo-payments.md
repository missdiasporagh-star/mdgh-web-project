...
Open Admin → Applications. Pending manual applications are now visible. Open the matching reference, check the actual incoming transfer in the receiving MoMo account, enter its transaction ID and GHS 230 received, and confirm. The server authenticates the staff session, checks request origin and amount, records an audit event, and emails the existing application magic link. Duplicate receipts and duplicate paid applications are blocked. If email delivery fails, repeat with the same receipt or have the applicant use the existing recovery page.
...
- Full Vitest suite: 110 passing tests, including new manual checkout and staff approval controls.
